...
class ResultGUI:
    def __init__(self, master, results, ai_response=None, speak_results=False):
        ...
        # Clear text area before displaying results
...
class SearchGUI:
        ...
        def search_thread():
            try:
                ...
                if not results and not ai_response:
                    self.root.after(0, lambda: self.update_status("No matches found for your query or prompt."))
                ...
                self.root.after(0, lambda: self.update_status(""))  # Clear status after showing results

            except Exception as e:
                self.root.after(0, lambda: self.update_status("An error occurred."))
    ...
    def search(self):
        ...
        self.update_status("Searching...")  # Set status to "Searching..."
    ...
    def show_results(self, results, ai_response=None):
        ...
        self.update_status("")  # Clear status once results are displayed
